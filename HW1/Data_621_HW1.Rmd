...
We will perform separate imputations in the following order, based on the number of missing values:
1. **TEAM_BATTING_SO**
2. **TEAM_BASERUN_SB**
3. **TEAM_FIELDING_DP**
4. **TEAM_BASERUN_CS**

Variables that are already imputed are included in the models after and un-imputed variables are not included.
We will keep the trained models and use them to impute missing values in the test data set.
...
lm_team_fld_dp <- lm(TEAM_FIELDING_DP ~ . - TARGET_WINS  - TEAM_BASERUN_CS, data = train_prepared_df)
...
lm_team_bas_cs <- lm(TEAM_BASERUN_CS ~ . - TARGET_WINS, data = train_prepared_df)
...
The residual plots and standardized residual plots show that the residuals are independent and approximately constant with mean 0 within the cloud of data.  
...
```{r}
print(paste0("Full model:                                    ",toString(summary(lm_win_full)$adj.r.squared)))
print(paste0("Exclude TEAM_BASERUN_CS:                       ",toString(summary(lm_win_eff_adj)$adj.r.squared)))
print(paste0("EXclude TEAM_BASERUN_CS and TEAM_FIELDING_DP:  ",toString(summary(lm_win_eff_adj2)$adj.r.squared)))
print(paste0("Exclude variables with missing values:         ",toString(summary(lm_win_exc_mis)$adj.r.squared)))

```
All models have Adjusted R-squared less than 0.31, we may have omitted variables that are not recorded in our data set.
The second model performs the best fitting the train data set. However, it doesn't mean that it will do well in predicting unseen data. We will test the performance of the models using a test data set. 
  

## * **RMSD (Root Mean Square Deviation)**   
The RMSD (Root Mean Square Deviation) is a measurement for the difference between a model's predicted values and the actual values. We will use the RMSD to compare the 4 models performance.  
  
First, let's look at the summary of our test data set:  
```{r}
summary(test_raw_df)
```
...
# 
# test_prepared_df[is.na(test_prepared_df$test_prepared_df),]$TEAM_BASERUN_SB <- predict(lm_team_bas_sb,test_prepared_df[is.na(test_prepared_df$TEAM_BASERUN_SB),])
# 
