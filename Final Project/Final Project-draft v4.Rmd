...
corrplot::corrplot(cor(train_df[,6:(ncol(train_df))], use = "na.or.complete"), 
...
*HP_visits_phour* is highly correlated with *free_ad_phour*. As we would like to know the user's option about the ads, we would keep *free_ad_phour* and drop *HP_visits_phour*
...
plot_level <- ggplot(train_df,aes(x=end_level,fill=churn))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('') + scale_y_continuous(labels = scales::percent)

plot_gender <- ggplot(train_df,aes(x=gender,fill=churn))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('') + scale_y_continuous(labels = scales::percent)

plot_userAgent <- ggplot(train_df,aes(x=userAgent,fill=churn))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('') + scale_y_continuous(labels = scales::percent)

plot_level+plot_gender+plot_userAgent+plot_layout(ncol = 2, guides = "collect")
```
...
plot_songs_phour <- ggplot(train_df, aes(x=log(songs_phour), color=churn)) + geom_density(na.rm =TRUE)
plot_tot_errs_phour <- ggplot(train_df, aes(x=log(tot_errs_phour), color=churn)) + geom_density(na.rm =TRUE)
plot_upgrades_phour <- ggplot(train_df, aes(x=log(upgrades_phour), color=churn)) + geom_density(na.rm =TRUE)
plot_downgrades_phour <- ggplot(train_df, aes(x=log(downgrades_phour), color=churn)) + geom_density(na.rm =TRUE)
plot_save_setting_phour <- ggplot(train_df, aes(x=log(save_setting_phour), color=churn)) + geom_density(na.rm =TRUE)
plot_song_ratio <- ggplot(train_df, aes(x=song_ratio, color=churn)) + geom_density(na.rm =TRUE)
plot_new_songs_ratio <- ggplot(train_df, aes(x=new_songs_ratio, color=churn)) + geom_density(na.rm =TRUE)
plot_pos_negative_ratio <- ggplot(train_df, aes(x=pos_negative_ratio, color=churn)) + geom_density(na.rm =TRUE)
plot_ad_per_song <- ggplot(train_df, aes(x=log(ad_per_song), color=churn)) + geom_density(na.rm =TRUE)
plot_paid_ad_ph <- ggplot(train_df, aes(x=log(paid_ad_ph), color=churn)) + geom_density(na.rm =TRUE)
plot_free_ad_ph <- ggplot(train_df, aes(x=log(free_ad_ph), color=churn)) + geom_density(na.rm =TRUE)
plot_percent_paid <- ggplot(train_df, aes(x=percent_paid, color=churn)) + geom_density(na.rm =TRUE)
plots_avg_session_duration <- ggplot(train_df, aes(x=avg_session_duration, color=churn)) + geom_density(na.rm =TRUE)

plot_songs_phour+plot_tot_errs_phour+plot_upgrades_phour+plot_downgrades_phour+
  plot_save_setting_phour+plot_song_ratio+plot_new_songs_ratio+
  plot_pos_negative_ratio+plot_ad_per_song+plot_paid_ad_ph+
  plot_free_ad_ph+plot_percent_paid+plots_avg_session_duration+
...
For *percent_paid*, users who do not change their account level (the ones staying 100% of the time free or 100% of the time paid) seem to have lower churn rate. They are content about service they are using.
The ones who switch their account level may still feel unsatisfying about the service.

*song_ratio*, *new_songs_ratio*, *avg_session_duration* do not show any obvious patterns about churning.

The above findings are just ideas we gee from the plots, the actual effect would need to be confirmed by our model analysis.





The data we have is not well-balanced. 
