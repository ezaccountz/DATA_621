...
active_train <- bind_rows(active_train, temp)
...
inactive_train <- bind_rows(inactive_train, temp)
...
```{r}
active_logi <- glm(cancel~.,family = binomial, active_train)
```
...
```{r}
inactive_logi <- glm(cancel~.,family = binomial, inactive_train)
```
