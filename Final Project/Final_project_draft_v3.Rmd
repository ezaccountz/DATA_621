...
```{r include=FALSE}
factor_columns <- c("page","auth","method","status","level","gender","userAgent", "location")
...
prepared_df$Cancellation_Confirmation <- as.factor(prepared_df$Cancellation_Confirmation)
prepared_df
```
...
prepared_df$tot_act_phour <- prepared_df$Total_Activities/prepared_df$duration_in_hours
prepared_df$songs_phour <- prepared_df$NextSong/prepared_df$duration_in_hours
prepared_df$tot_tu_phour <- prepared_df$Thumbs_Up/prepared_df$duration_in_hours
prepared_df$tot_td_phour <- prepared_df$Thumbs_Down/prepared_df$duration_in_hours
prepared_df$frds_added_phour <- prepared_df$Add_Friend/prepared_df$duration_in_hours
prepared_df$tot_add2PL_phour <- prepared_df$Add_to_Playlist/prepared_df$duration_in_hours
prepared_df$HP_visits_phour <- prepared_df$Home/prepared_df$duration_in_hours
prepared_df$HP_visits_phour <- prepared_df$Home/prepared_df$duration_in_hours
prepared_df$tot_ads_phour <- prepared_df$Roll_Advert/prepared_df$duration_in_hours
prepared_df$tot_errs_phour <- prepared_df$Error/prepared_df$duration_in_hours
prepared_df$upgrade_downgrades_phour <- (prepared_df$Submit_Upgrade + prepared_df$Submit_Downgrade)/prepared_df$duration_in_hours
prepared_df$upgrade_downgrades <- (prepared_df$Submit_Upgrade + prepared_df$Submit_Downgrade)
prepared_df$song_ratio <- prepared_df$NextSong / prepared_df$Total_Activities
prepared_df$new_songs_ratio <- prepared_df$new_songs_listened / prepared_df$NextSong
prepared_df$pos_negative_ratio <- prepared_df$Thumbs_Up/(prepared_df$Thumbs_Down + 0.0001)
prepared_df$tot_recent_act_phour  <- prepared_df$recent_total_act / lookback_window / 24
prepared_df$tot_oldest_act_phour  <- prepared_df$oldest_total_act / lookback_window / 24
prepared_df$recent_songs_phour  <- prepared_df$recent_total_song / lookback_window / 24
prepared_df$oldest_songs_phour  <- prepared_df$oldest_total_song / lookback_window / 24
prepared_df
```
...
```{r}
prepared_df1 <- merge(prepared_df, session_avg, by=c("userId")) %>% 
                arrange(userId)
  
prepared_df2 <- merge(prepared_df1, session_avg_length, by=c("userId")) %>% 
                arrange(userId)
prepared_df3 <- merge(prepared_df2, window_home_songs, by=c("userId")) %>% 
                arrange(userId)
  
prepared_df4 <- merge(prepared_df3, window_ads_songs, by=c("userId")) %>% 
                arrange(userId)
prepared_df4
...
```{r}
ggplot(prepared_df4 %>% filter(Cancellation_Confirmation == 1), aes(x = end_level)) + 
...
```{r}
ggplot(prepared_df4 %>% filter(Cancellation_Confirmation== 1), aes(x=fct_infreq(location))) + 
    geom_bar()+
    theme_classic() + 
    theme(axis.text.x = element_text(angle = 90)) + 
    labs(x = "state", 
...
```{r}
ggplot(prepared_df4 %>% filter(Cancellation_Confirmation == 1), aes(x = gender))+
  geom_bar(na.rm = T) +
  theme_classic() + 
...
```{r}
correlation = cor(prepared_df %>% select(-c(end_level, gender, userAgent, location, userId)), use = 'pairwise.complete.obs')
corrplot::corrplot(correlation, 'ellipse', type = 'lower',  order = 'hclust')
```
**distribution of activity variables**
```{r warning=FALSE}
temp <- prepared_df4 %>% select(-c(userId))
temp %>% 
  purrr::keep(is.numeric) %>% 
  cbind(Cancellation_Confirmation = as.factor(prepared_df4$Cancellation_Confirmation)) %>% 
  gather() %>% 
